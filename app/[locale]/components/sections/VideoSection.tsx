...
export default function VideoSection({colorSchema , dictionary} : VideoSectionPropsInterface ){
    return (
        <section id="video" className={`relative w-full md:h-100 lg:h-200 lg:px-14 lg:pt-14 ${colorSchema.tertiaryBg}`}>
            ...
            <div className="absolute bottom-2 left-2 lg:left-20 lg:bottom-20 bg-black/10 p-2 md:bg-transparent md:p-0">
                <p className={`text-3xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-9xl ${colorSchema.secondaryText} tracking-wide uppercase drop-shadow-2xl`}>{dictionary.videoSection.title}:</p>
                <p className={`text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-7xl ${colorSchema.secondaryText} tracking-wide drop-shadow-2xl`}>{dictionary.videoSection.subtitle}</p>
            </div>
        </section>
    )
}